...
Feature: Agent CVM base image
  ...
  Background:
    ...
    And the Agent CVM should receive only scoped unlock material for the lease
  ...
  Scenario: Agent CVM waits locked until phone or KMS release
    Given the Agent CVM has started and attested
    When no valid unlock payload has arrived
    Then it can serve only attestation, health, and claim endpoints
    And it cannot mount encrypted state
    And it cannot perform mutating external actions as the AgentIdentity

  Scenario: Agent CVM mounts only the latest permitted state
    Given phone/KMS releases a scoped storage unlock lease
    When the Agent CVM attempts to mount encrypted state
